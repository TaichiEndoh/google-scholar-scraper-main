...
def make_url(keyword, conf, author, year, paper_id=None):
    ...
    :return: url
    """
    #assert文で 条件式がTrueではない時に、例外を投げます
    assert (
        keyword is not None
        or conf is not None
...
def get_snippet(soup):
    ...
    :return: snippet_list
    スニペットは
    検索後の結果ページで表示されるWebページの説明文
    The description of the web page that will be displayed on the results page after a search.
    """
